...
public class BoardDAO {
	...
	public void insert(BoardDTO dto) {
		...
		ResultSet rs = null;
		String sql = "insert into board(num,id,title,content,reproot,repstep,repindent) values(?,?,?,?,?,0,0)";
		try {
	...
	public BoardDTO read(int num) {
		...
		try {
			...
			if(rs.next()) {
				...
				dto = new BoardDTO(num, id, title, content, writeday, readcnt, 0, 0, 0);
			}
	...
	public PageTO list(int curPage) {
		...
		try {
			...
			while(rs.next()) {
				...
				System.out.print(dto.getRepStep());
				list.add(dto);
			}
